...
export default function Review() {
    ...
    const { currentStep, setCurrentStep, setTotalData, setCompletedSteps, totalData } =
    ...
    useEffect(() => {
        ...
        if (user && user._id) {
            ...
            const personalData = localStorage.getItem(`${user._id}_StartupOwnerPersonalInfo`);
            // Organization Information
            const organizationData = localStorage.getItem(`${user._id}_StartupOwnerOrganizationInfo`);
            // Financial Information
            const financialData = localStorage.getItem(`${user._id}_StartupOwnerFinancialInfo`);
            // Banking Information
            const bankingData = localStorage.getItem(`${user._id}_StartupOwnerBankingInfo`);
            // Documents
            const documentsData = localStorage.getItem(`${user._id}_StartupOwnerDocuments`);
            const uploadedDocsData = localStorage.getItem(`${user._id}_UploadedDocs`);
    ...
    async function handleFinalSubmit(e) {
        try {
            ...
            const result = await startupRegistrationApplicationService.submitApplication(submissionData);
    ...
    return (
        <div className="p-6 w-full bg-blue-50 overflow-x-scroll rounded-lg shadow-md border border-gray-200">
                    ...
                    <div className="space-y-4">
                        ...
                        {applicationData.personal && (
                            <div className="bg-white p-4 rounded-lg shadow">
                                ...
                                <p><strong>Name:</strong> {applicationData.personal.firstName} {applicationData.personal.lastName}</p>
                                <p><strong>Email:</strong> {applicationData.personal.email}</p>
                                <p><strong>Phone:</strong> {applicationData.personal.phoneNumber}</p>
                                <p><strong>Status:</strong> {applicationData.personal.personalInfoStatus || 'pending'}</p>
                            </div>
                        )}
                        ...
                        {applicationData.organization && (
                            <div className="bg-white p-4 rounded-lg shadow">
                                <h3 className="font-bold text-lg mb-2">Organization Information</h3>
                                <p><strong>Organization Name:</strong> {applicationData.organization.organizationName}</p>
                                <p><strong>Type:</strong> {applicationData.organization.organizationType}</p>
                                <p><strong>Address:</strong> {applicationData.organization.address}</p>
                                <p><strong>Status:</strong> {applicationData.organization.organizationInfoStatus || 'pending'}</p>
                            </div>
                        )}
                        ...
                        {applicationData.financial && (
                            <div className="bg-white p-4 rounded-lg shadow">
                                ...
                                <p><strong>Startup Name:</strong> {applicationData.financial.startupName}</p>
                                <p><strong>Revenue:</strong> ₹{applicationData.financial.revenue} Cr</p>
                                <p><strong>Valuation:</strong> ₹{applicationData.financial.valuation} Cr</p>
                                <p><strong>Status:</strong> {applicationData.financial.financialInfoStatus || 'pending'}</p>
                            </div>
                        )}
                        ...
                        {applicationData.banking && (
                            <div className="bg-white p-4 rounded-lg shadow">
                                <h3 className="font-bold text-lg mb-2">Banking Information</h3>
                                ...
                                <p><strong>IFSC Code:</strong> {applicationData.banking.ifscCode}</p>
                                <p><strong>Status:</strong> {applicationData.banking.bankingInfoStatus || 'pending'}</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <div className="mb-6">
                <h3 className="font-bold text-lg mb-4 text-center">Uploaded Documents</h3>
                    ...
                    {documentTypes.map((doc) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() => hasDocument && handleViewDocument(doc.key)}
                            >
                                <div className={`text-4xl mb-2 ${hasDocument ? 'text-blue-600' : 'text-gray-400'}`}>
                                    📄
                                </div>
                                <p className="text-xs text-center">{doc.label}</p>
                                {hasDocument && (
                ...
                {applicationData?.documents?.documentsStatus && (
                    <p className="text-center mt-4 text-green-600 font-medium">
                        Documents Status: {applicationData.documents.documentsStatus}
                    </p>
                )}
            </div>
            {/* submit btn */}
